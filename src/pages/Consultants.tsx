...
import { consultants as mockConsultants, getConsultantById, getAllocationsForConsultant, projects as mockProjects, allocations as mockAllocations } from '../data/mockData';
...
const Consultants: React.FC = () => {
  ...
  const consultantAllocations = id ? getCurrentAllocationsForConsultant(id) : [];
  ...
  const handleConfirmMove = (data: {
    ...
    const updatedConsultants = localConsultants.map(c => c.id === data.consultantId ? {
      ...c,
      status: 'Allocated' as const,
      currentProject: data.projectId
    } : c);
    setLocalConsultants(updatedConsultants);
    const updatedProjects = localProjects.map(p => p.id === data.projectId ? {
      ...p,
      resourcesAssigned: p.resourcesAssigned + 1,
      staffingStatus: p.resourcesAssigned + 1 >= p.resourcesNeeded ? 'Fully Staffed' as const : 'Needs Resources' as const
    } : p);
  ...
  const handleDeleteAllocation = (allocationId: string) => {
    ...
    if (remainingAllocations.length === 0) {
      const updatedConsultants = localConsultants.map(c => c.id === allocation.consultantId ? {
        ...c,
    ...
    const updatedProjects = localProjects.map(p => p.id === allocation.projectId ? {
      ...p,
      resourcesAssigned: Math.max(0, p.resourcesAssigned - 1),
      staffingStatus: p.resourcesAssigned - 1 < p.resourcesNeeded ? 'Needs Resources' as const : 'Fully Staffed' as const
    } : p);
    setLocalProjects(updatedProjects);
    toast({
  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        {id && consultant ? <ConsultantDetail consultant={consultant} allocations={consultantAllocations} projects={localProjects} onMoveConsultant={handleMoveConsultant} onDeleteAllocation={handleDeleteAllocation} onDeleteConsultant={handleDeleteConsultant} /> : <>
            ...
            <ConsultantList consultants={localConsultants} onDeleteConsultant={handleDeleteConsultant} />
          </>}
      </main>
      
      {consultant && <MoveConsultantModal isOpen={showMoveModal} onClose={() => setShowMoveModal(false)} onConfirm={handleConfirmMove} consultants={[consultant]} project={null} type="fromProject" preselectedConsultant={consultant} />}
      
      <AddConsultantDialog isOpen={showAddDialog} onClose={() => setShowAddDialog(false)} onSaveConsultant={handleSaveConsultant} />
    </div>;
};
