...
export const ConsultantsList = ({
  ...
  useEffect(() => {
    ...
    const fetchRankedConsultants = async () => {
      try {
        ...
        const rankedConsultantsList = (rankingResult.ranking as RankingItem[]).map(r => {
  ...
  return <div className="bg-white rounded-lg shadow p-4 h-full relative">
      <div className="mb-2 flex items-center justify-between">
        <h3 className="font-semibold inline-block text-xl">Available Consultants</h3>
        <ToggleGroup type="single" value={teamStructure} onValueChange={value => value && setTeamStructure(value as TeamStructure)} className="flex justify-end">
      ...
      {selectedProjectId && <div className="mb-4">
          <div className="flex items-center mb-2 gap-2">
            <p className="text-sm text-gray-500 mb-0">Team Structure:</p>
            {rankingResult?.structure && <span className="text-xs bg-gray-100 text-gray-800 px-2 py-1 rounded-full">
      ...
      <div className="mb-4 flex items-center gap-2 relative">
        ...
        <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Search consultants..." />
        <select value={seniorityFilter} onChange={e => setSeniorityFilter(e.target.value as SeniorityFilter)} className="ml-2 px-3 py-1.5 border border-gray-300 rounded-lg bg-white text-sm text-gray-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
      ...
      {selectedProjectId ? <ScrollArea className="h-[calc(100%-150px)] pr-4">
        <div className="space-y-2  pb-4">
          {filteredConsultants.length === 0 ? <p className="text-gray-500 text-center py-4">No consultants available matching your criteria</p> : filteredConsultants.map(consultant => {
          ...
          return <div key={consultant.id} className={`p-2 rounded-lg mb-1 transition-all flex items-center min-h-0 ${isSelected ? 'bg-blue-50 border-l-4 border-blue-500 shadow-sm' : 'hover:bg-gray-50 hover:shadow-sm'}`}>
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-500 flex items-center space-x-2">
                          ...
                          <div className="flex space-x-1">
                            {consultant.expertise.split(',').map((exp, i) => <span key={i} className="text-xs">
                                {exp.trim()}
                              </span>)}
                          </div>
                        </div>
                  ...
                  <div className="flex items-center space-x-4 ml-auto">
                    ...
                    <div className="w-20">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-1.5">
                        <div className={`${getBgColorForAllocation(allocationPercentage)} h-1.5 rounded-full`} style={{
                    width: `${allocationPercentage}%`
                  }} />
                      </div>
                    </div>
                    <Button onClick={() => onAllocateConsultant(consultant)} variant={isSelected ? "destructive" : "outline"} size="sm" className="transition-all">
                      {isSelected ? <>
      ...
      </ScrollArea> : <div className="h-full flex items-center justify-center">
